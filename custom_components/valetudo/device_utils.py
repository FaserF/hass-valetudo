...
async def async_enrich_registry(hass: HomeAssistant, device_id: str, vacuum_entity_id: str):
    ...
    if mac:
        ...
        if not any(conn[1] == formatted_mac for conn in device.connections):
            ...
            _LOGGER.info(f"Enriched Valetudo device {device.name} ({device_id}) with MAC {formatted_mac}")
    else:
        _LOGGER.debug(f"Could not find MAC for Valetudo device {device_id}")

def _async_extract_mac_from_identifiers(identifiers: set[tuple[str, str]]) -> str | None:
